...
async function sendTicketSetupPanel(channel) {
    ...
    const defaultBanner = 'https://i.ibb.co/L5BwY3q/banner.png';

    const embed = new EmbedBuilder()
        .setColor('#5865F2')
        .setTitle(settings?.panelTitle || '✨ NEVA DESTEK MERKEZİ')
        .setDescription(settings?.panelDescription || 
            `> **Sunucumuzda yardım ve destek almak için aşağıdaki menüyü kullanabilirsiniz.**\n> İhtiyacınıza uygun kategoriyi seçerek özel destek biletinizi anında başlatın.`
        )
        .addFields(
            { name: '⏰ Mesai Saatleri', value: '`10:00 - 22:00`', inline: true },
            { name: '🔒 Gizlilik', value: '`Uçtan Uca Yedekli`', inline: true },
            { name: '⚡ Yanıt Süresi', value: '`Ort. 15 Dakika`', inline: true }
        )
        .setImage(settings?.panelBanner || defaultBanner)
        .setFooter({ text: 'Neva Ticket System • Kaliteli Destek Deneyimi', iconURL: channel.guild.iconURL({ dynamic: true }) })
        .setTimestamp();

    const selectMenu = new StringSelectMenuBuilder()
        .setCustomId('ticket_select_category')
        .setPlaceholder('👉 Destek Almak İstediğiniz Kategoriyi Seçin...')
        .addOptions([
            {
                label: 'Teknik & Sistem Desteği',
                value: 'tech',
                description: 'Yazılım, sunucu ve teknik sorunlar için anında destek',
                emoji: '💻'
            },
            {
            ...
            },
            {
                ...
                value: 'complaint',
                description: 'Kural ihlali yapan kullanıcı veya yetkili bildirimi',
                emoji: '⚠️'
            },
            {
                label: 'Genel Soru & İletişim',
                value: 'general',
                description: 'Aklınıza takılan her türlü soru ve bilgi talebi',
                emoji: '❓'
            }
        ]);
