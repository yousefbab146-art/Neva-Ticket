...
/**
 * Bilet Kurulum Mesajını Gönderir (/ticket-kur)
 */
async function sendTicketSetupPanel(channel) {
    ...
    const embed = new EmbedBuilder()
        .setColor('#5865F2')
        .setTitle(settings?.panelTitle || '🎟️ Neva Destek Talebi Oluştur')
        .setDescription(settings?.panelDescription || 'Aşağıdaki menüden yardım almak istediğiniz konuyu seçerek hızlıca bilet açabilirsiniz.')
        .setFooter({ text: 'Neva Ticket & Support System' })
        .setTimestamp();

    if (settings?.panelBanner) {
        embed.setImage(settings.panelBanner);
    }

    const selectMenu = new StringSelectMenuBuilder()
        .setCustomId('ticket_select_category')
        .setPlaceholder('👉 Yardım Almak İstediğiniz Konuyu Seçin...')
        .addOptions([
            { label: 'Teknik Destek', value: 'tech', description: 'Yazılım ve teknik sorunlar için destek alın', emoji: '💻' },
            { label: 'Satın Alım / Ödeme', value: 'billing', description: 'Ödeme ve VIP üyelik işlemleri', emoji: '💳' },
            { label: 'Şikayet & Bildirim', value: 'complaint', description: 'Kullanıcı veya yetkili şikayetleri', emoji: '⚠️' },
            { label: 'Genel Sorular', value: 'general', description: 'Genel bilgi ve sorularınız', emoji: '❓' }
        ]);
